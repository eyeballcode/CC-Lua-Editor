...
public class NewFileOptionFrame extends JFrame {
		...
		b.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (text.trim().equals("")) {
					...
					boolean shouldComplain = new File(Utils.workingDir + "Workspace/" + prjName +"/" +  text).exists();
					System.out.println(shouldComplain);
					if(!shouldComplain) {
						Utils.createFile((Utils.workingDir + "Workspace/" + prjName +"/" +  text));
				System.out.println("[INFO] " + new Date()
						+ ": File " + Utils.workingDir + "Workspace/" + prjName + "/" + text +  " created!");
				setVisible(false);
				System.out.println(prjName);
				System.out.println("[INFO] " + new Date()
				+ ": Don't worry, this exception is normal!");
				System.out.println("[INFO] " + new Date()
				+ ": It means the file " + text +" does not exist!");
				ProjectEditorScreen.tp.open(prjName, text);
				new ProjectEditorScreen().setOppositeVisible();
				
					} else {
						System.out.println("[ERROR] " + new Date()
						+ ": File " + text +" already exists!");
				JOptionPane.showMessageDialog(new JFrame(),
						"File " + text +" already exists!",
						"ERROR", JOptionPane.ERROR_MESSAGE);
				tf.requestFocus();
	...
	public NewFileOptionFrame(String prjName) {
